...
| `manifest` | outputs match the manifest | A4.3 rules; artifact IDs unique; `paper/_variables.yml` equals the merge of the variables files and matches `variables_sha256` | fail | check | outputs are covered by provenance records and consistent with each other |
