...
| E16 | The generated project's workflow renders and checks on `ubuntu-24.04` and uploads `paper-preview` on push; `provision` pushes, creates the issues, and skips them on a rerun; `gh run list` and the artifacts API report the run and the artifact | manual: end-to-end run against a private scratch repository (preview contained both formats, the manifest, and `build-info.json`) | 2026-09-25 |
